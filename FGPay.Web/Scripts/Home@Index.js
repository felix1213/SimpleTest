...
function GetLoadNav() {
    var data = top.clients.menuPermissions;
    var _html = "";
    var mainMenu = $.grep(data, function (x, i) {
        return x.parentid == 0;
    });
    $.each(mainMenu, function (j) {
        var row = mainMenu[j];
        _html += '<li>';
        _html += '<a data-id="' + row.id + '" href="#" class="dropdown-toggle"><i class="fa fa-tags"></i><span>' + row.name + '</span><i class="fa fa-angle-right drop-icon"></i></a>';
        var childNodes = $.grep(data, function (x, i) {
            return x.parentid == row.id;
        });
        if (childNodes.length > 0) {
            ...
            $.each(childNodes, function (i) {
                ...
                _html += '<li>';
                _html += '<a class="menuItem" data-id="' + subrow.id + '" href="/' + subrow.controller + '/' + subrow.action +'" data-index="' + subrow.code + '">' + subrow.name + '</a>';
                _html += '</li>';
            });
            _html += '</ul>';
        }
    ...
    var userInfo = top.clients.userInfo;
    $("#sUid1").text(userInfo.u);
    $("#sUid2").text(userInfo.u);
}